...
bool PassMultiStageSplitter::run(
  ...
  if(!context->getOptions().Debug) {
    ...
    for(auto& stencil : stencilInstantiation->getStencils()) {
      ...
      for(auto multiStageIt = stencil->getMultiStages().begin();
        ...
        for(auto stageIt = multiStage.getStages().rbegin();
            stageIt != multiStage.getStages().rend(); ++stageIt, --stageIndex) {
          Stage& stage = (**stageIt);
  ...
  // If we compile with the -fdebug option, we turn off all the optimization and put every statement
  // in its own multistage. This is more snychronization than would be needed but allows for very
  // easy code-inspection and does not rely on a good splitting algorithm. This option will lead to
  // bad performance and is only tought to be used to fix bugs
  else {
    int numSplit = 0;
    std::deque<MultiStage::SplitIndex> splitterIndices;
