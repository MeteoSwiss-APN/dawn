...
public:
  AddBoundaryConditions(iir::IIR* internalIR, int StencilID)
      : iir_(internalIR), StencilID_(StencilID) {}

  void visit(const std::shared_ptr<StencilCallDeclStmt>& stmt) override {
    auto iter = std::find_if(
        iir_->getMetaData()->getStencilCallToStencilIDMap().begin(),
        iir_->getMetaData()->getStencilCallToStencilIDMap().end(),
        [this, stmt](const std::pair<std::shared_ptr<StencilCallDeclStmt>, int>& pair) {
          return pair.first == stmt && pair.second == StencilID_;
        });
    if(iter != iir_->getMetaData()->getStencilCallToStencilIDMap().end()) {
...
bool PassSetBoundaryCondition::run(
  ...
  if(stencilInstantiation->getIIR()->getChildren().size() == 1) {
    if(stencilInstantiation->getIIR()->getOptions().ReportBoundaryConditions) {
      std::cout << "\nPASS: " << getName() << ": "
  ...
  auto getOriginalID = [&](int ID) {
    ...
    auto checkIfFieldWasOriginallyDefined = [&](int fieldID) {
      auto it = stencilInstantiation->getIIR()->getMetaData()->getNameToAccessIDMap().find(
          getOriginalNameFromAccessID(fieldID, stencilInstantiation->getIIR()));
    ...
    if(checkIfFieldWasOriginallyDefined(ID)) {
      if(stencilInstantiation->getIIR()->getMetaData()->isField(ID)) {
        return stencilInstantiation->getIIR()->getMetaData()->getAccessIDFromName(
            getOriginalNameFromAccessID(ID, stencilInstantiation->getIIR()));
      } else {
  ...
  std::transform(
      stencilInstantiation->getIIR()->getMetaData()->getBoundaryConditions().begin(),
      stencilInstantiation->getIIR()->getMetaData()->getBoundaryConditions().end(),
      std::inserter(allBCs, allBCs.begin()),
      [&](std::pair<std::string, std::shared_ptr<BoundaryConditionDeclStmt>> bcPair) {
        return std::make_pair(
            stencilInstantiation->getIIR()->getMetaData()->getAccessIDFromName(bcPair.first),
            bcPair.second);
      });
  ...
  for(const auto& stencilcall : findStencilCalls.getStencilCalls()) {
    DAWN_ASSERT_MSG(
        stencilInstantiation->getIIR()->getMetaData()->getStencilCallToStencilIDMap().count(
            stencilcall),
        "no StencilID for the stencilcall found");
    StencilIDsVisited_.emplace(
        stencilInstantiation->getIIR()->getMetaData()->getStencilCallToStencilIDMap()[stencilcall]);
  }
  ...
  auto calculateHaloExtents = [&](std::string fieldname) {
    ...
    for(const auto& stencil : stencilInstantiation->getIIR()->getChildren()) {
      if(stencilIDsToVisit.count(stencil->getStencilID())) {
        fullExtent.merge(analyzeStencilExtents(
            stencil,
            stencilInstantiation->getIIR()->getMetaData()->getAccessIDFromName(fieldname)));
        if(StencilBCsApplied_.count(fieldname) == 0) {
  ...
  for(const auto& stencilPtr : stencilInstantiation->getIIR()->getChildren()) {
    ...
    DAWN_LOG(INFO) << "analyzing stencil "
                   << stencilInstantiation->getIIR()->getMetaData()->getName();
    ...
    for(const auto& stmtAccess : iterateIIROver<iir::StatementAccessesPair>(stencil)) {
      ...
      for(const auto& readaccess : allReadAccesses) {
        ...
        if(IDtoBCpair == allBCs.end()) {
          DAWN_ASSERT_MSG(
              ...
              dawn::format("In stencil %s we need a halo update on field %s but no "
                           ...
                           stencilInstantiation->getIIR()->getMetaData()->getName(),
                           getOriginalNameFromAccessID(originalID, stencilInstantiation->getIIR()),
                           getOriginalNameFromAccessID(originalID, stencilInstantiation->getIIR()))
                  .c_str());
        }
        // Calculate the extent and add it to the boundary-condition - Extent map
        iir::Extents fullExtents = calculateHaloExtents(
            stencilInstantiation->getIIR()->getMetaData()->getNameFromAccessID(readaccess.first));
        stencilInstantiation->getIIR()->getMetaData()->insertBoundaryConditiontoExtentPair(
            IDtoBCpair->second, fullExtents);
        ...
        AddBoundaryConditions visitor(stencilInstantiation->getIIR().get(), stencil.getStencilID());
  ...
  if(stencilInstantiation->getIIR()->getOptions().ReportBoundaryConditions) {
    ...
    for(const auto& ID : boundaryConditionInserted_) {
      std::cout << " Boundary Condition for field '"
                << getOriginalNameFromAccessID(ID, stencilInstantiation->getIIR()) << "' inserted"
                << std::endl;
    }
  }
