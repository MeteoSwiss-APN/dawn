...
bool PassSetBoundaryCondition::run(const std::unique_ptr<iir::IIR>& iir) {
  ...
  if(iir->getChildren().size() == 1) {
    if(iir->getOptions().ReportBoundaryConditions) {
      std::cout << "\nPASS: " << getName() << ": "
  ...
  auto getOriginalID = [&](int ID) {
    ...
    auto checkIfFieldWasOriginallyDefined = [&](int fieldID) {
      auto it = iir->getMetaData()->getNameToAccessIDMap().find(
          getOriginalNameFromAccessID(fieldID, iir));
      return it != iir->getMetaData()->getNameToAccessIDMap().end();
    };
    ...
    if(checkIfFieldWasOriginallyDefined(ID)) {
      if(iir->getMetaData()->isField(ID)) {
        return iir->getMetaData()->getAccessIDFromName(
            getOriginalNameFromAccessID(ID, iir));
      } else {
  ...
  std::transform(
      iir->getMetaData()->getBoundaryConditions().begin(),
      iir->getMetaData()->getBoundaryConditions().end(),
      std::inserter(allBCs, allBCs.begin()),
      [&](std::pair<std::string, std::shared_ptr<BoundaryConditionDeclStmt>> bcPair) {
        return std::make_pair(
            iir->getMetaData()->getAccessIDFromName(bcPair.first),
            bcPair.second);
      });
  ...
  for(const auto& stencilcall : findStencilCalls.getStencilCalls()) {
    DAWN_ASSERT_MSG(
        iir->getMetaData()->getStencilCallToStencilIDMap().count(
            stencilcall),
        "no StencilID for the stencilcall found");
    StencilIDsVisited_.emplace(
        iir->getMetaData()->getStencilCallToStencilIDMap()[stencilcall]);
  }
    ...
    for(const auto& stencil : iir->getChildren()) {
      if(stencilIDsToVisit.count(stencil->getStencilID())) {
        fullExtent.merge(analyzeStencilExtents(
            stencil,
            iir->getMetaData()->getAccessIDFromName(fieldname)));
        if(StencilBCsApplied_.count(fieldname) == 0) {
  ...
  for(const auto& stencilPtr : iir->getChildren()) {
    ...
    for(const auto& stmtAccess : iterateIIROver<iir::StatementAccessesPair>(stencil)) {
      ...
      for(const auto& readaccess : allReadAccesses) {
        ...
        AddBoundaryConditions visitor(iir.get(), stencil.getStencilID());
  ...
  if(iir->getOptions().ReportBoundaryConditions) {
    ...
    for(const auto& ID : boundaryConditionInserted_) {
      std::cout << " Boundary Condition for field '"
                << getOriginalNameFromAccessID(ID, iir) << "' inserted"
                << std::endl;
    }
  }
