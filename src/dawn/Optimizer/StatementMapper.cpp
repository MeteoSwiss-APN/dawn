...
StatementMapper::StatementMapper(
    ...
    : stencilFunctions_(stencilFunctions), instantiation_(instantiation),
      metadata_(instantiation->getMetaData()), stackTrace_(stackTrace) {
...
void StatementMapper::appendNewStatementAccessesPair(const std::shared_ptr<iir::Stmt>& stmt) {
  ...
  if(scope_.top()->ScopeDepth == 1) {
    ...
    stmt->StackTrace =
        stackTrace_ ? std::make_shared<std::vector<ast::StencilCall*>>(*stackTrace_) : nullptr;
...
void StatementMapper::visit(const std::shared_ptr<iir::StencilFunCallExpr>& expr) {
  ...
  for(auto& iirStencilFun : stencilFunctions_) {
