...
//public:
//  StencilDescStatementMapper(iir::IIR* iir, sir::Stencil* sirStencil)
//      : iir_(iir), sirStencil_(sirStencil) {
//    DAWN_ASSERT(iir);
...
//    iir_->insertChild(make_unique<Stencil>(nullptr, sirStencil_, StencilID), iir_->getMetaData()->nextUID());
...
//        nullptr,
//        /// TODO: REPLACE WITH IIR LATER ///
//        ///
//        ///
//        verticalRegion->LoopOrder == sir::VerticalRegion::LK_Forward ? LoopOrderKind::LK_Forward
...
//    // TODO: nullptr passed
//    StatementMapper statementMapper(nullptr, scope_.top()->StackTrace, doMethod,
...
bool OptimizerContext::fillIIRFromSIR(std::unique_ptr<iir::IIR>& iir,
  ...
  for(const auto& field : SIRStencil->Fields) {
    std::cout << "inserting field: " << field->Name << std::endl;
...
//  StencilDescStatementMapper stencilDeclMapper(iir.get(), SIRStencil);

  //  StencilDescStatementMapper stencilDeclMapper(iir.get(), iir->getMetaData().get(),
  //  SIRStencil->Name,
  //                                               iir->getMetaData()->getStencilDescStatements(),
  //                                               iir->getMetaData()->getNameToAccessIDMap());
  ...
  //  PassTemporaryType::fixTemporariesSpanningMultipleStencils(iir->getMetaData().get(),
  //                                                            iir->getMetaData()->getStencils());

  //  if(iir->getMetaData()->getOptimizerContext()->getOptions().ReportAccesses)
