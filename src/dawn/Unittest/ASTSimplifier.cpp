...
std::shared_ptr<sir::VarDeclStmt> vecdecl(const std::string& type, const std::string& name,
                                     std::vector<std::shared_ptr<sir::Expr>> initList, int dimension,
                                     const char* op) {
...
std::shared_ptr<sir::FieldAccessExpr> field(const std::string& name, Array3i offset, Array3i argumentMap,
                                       Array3i argumentOffset, bool negateOffset) {
  return std::make_shared<sir::FieldAccessExpr>(name, offset, argumentMap, argumentOffset, negateOffset);
}
