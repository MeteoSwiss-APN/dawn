...
void serializeMetaData(proto::iir::StencilInstantiation& target,
                       iir::StencilMetaInformation& metaData) {
  proto::iir::StencilMetaInfo protoMetaData;
  target.set_allocated_metadata(&protoMetaData);
}

void serializeIIR(proto::iir::StencilInstantiation& target, const std::unique_ptr<iir::IIR>& iir) {
  proto::iir::IIR protoIIR;
  ...
  target.set_allocated_internalir(&protoIIR);
}

std::string serializeImpl(const std::shared_ptr<iir::StencilInstantiation>& instantiation,
...
std::unique_ptr<dawn::iir::IIR> deserializeImpl(const std::string& str,
  ...
  switch(kind) {
  case dawn::IIRSerializer::SK_Json: {
    auto status = google::protobuf::util::JsonStringToMessage(str, &protoIIR);
    if(!status.ok())
      throw std::runtime_error(dawn::format("cannot deserialize IIR: %s", status.ToString()));
    break;
  }
  case dawn::IIRSerializer::SK_Byte: {
    if(!protoIIR.ParseFromString(str))
      throw std::runtime_error(dawn::format("cannot deserialize IIR: %s")); //,
  ...
  std::unique_ptr<iir::IIR> iir = make_unique<iir::IIR>();

  for(const auto& protoStencils : protoIIR.stencils()) {
    //        iir->insertChild(make_unique<iir::Stencil>());
...
std::unique_ptr<dawn::iir::IIR>
dawn::IIRSerializer::deserialize(const std::string& file,
  ...
  return deserializeImpl(str, kind, instantiation);
}

std::unique_ptr<dawn::iir::IIR>
dawn::IIRSerializer::deserializeFromString(const std::string& str,
                                           std::shared_ptr<iir::StencilInstantiation> instantiation,
                                           dawn::IIRSerializer::SerializationKind kind) {
  return deserializeImpl(str, kind, instantiation);
}
