          ...
          for(int stmtidx = 0, stmtSize = lhsDoMethod->getChildren().size(); stmtidx < stmtSize;
            ...
            if(lhsStmt->getCallerAccesses()) {
              for(const auto& lhsPair : rhsStmt->getCallerAccesses()->getReadAccesses()) {
                assert(rhsStmt->getCallerAccesses()->getReadAccesses().count(lhsPair.first));
              ...
              for(const auto& lhsPair : rhsStmt->getCallerAccesses()->getWriteAccesses()) {
                assert(rhsStmt->getCallerAccesses()->getWriteAccesses().count(lhsPair.first));
...
static bool compareDerivedInformation(iir::IIR* lhs, iir::IIR* rhs) {
  ...
  //   /// Set containing the AccessIDs of fields which are manually allocated by the stencil and
  //   serve
  //   /// as temporaries spanning over multiple stencils
  // };

  // assert(lhs->getStageIDToNameMap() == rhs->getStageIDToNameMap());  //this is not present in
  // stencil from memory
  ...
  for(int stencils = 0, size = lhs->getChildren().size(); stencils < size; ++stencils) {
    ...
    assert(lhsStencil->getStageDependencyGraph() ==
           rhsStencil->getStageDependencyGraph()); // this _should_ work, to be tested
...
static void readIIRFromFile(OptimizerContext& optimizer,
...
static void compareIIRs(std::shared_ptr<iir::StencilInstantiation> lhs,
...
void deserialization_test_mat() {
  ...
  std::shared_ptr<iir::StencilInstantiation> copy_stencil_from_file =
      std::make_shared<iir::StencilInstantiation>(*optimizer.getSIR()->GlobalVariableMap,
                                                  optimizer.getSIR()->StencilFunctions);
  readIIRFromFile(optimizer, copy_stencil_from_file, "test_iirs/copy_stencil.iir");
  UIDGenerator::getInstance()->reset();

  std::shared_ptr<iir::StencilInstantiation> lap_stencil_from_file =
      std::make_shared<iir::StencilInstantiation>(*optimizer.getSIR()->GlobalVariableMap,
                                                  optimizer.getSIR()->StencilFunctions);
  ...
  std::shared_ptr<iir::StencilInstantiation> lap_stencil_memory =
      std::make_shared<iir::StencilInstantiation>(*optimizer.getSIR()->GlobalVariableMap,
                                                  optimizer.getSIR()->StencilFunctions);
