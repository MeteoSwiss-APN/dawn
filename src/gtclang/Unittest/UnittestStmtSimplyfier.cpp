...
std::shared_ptr<dawn::VarDeclStmt> vecdec(const std::string& type, const std::string& name,
                                    std::vector<std::shared_ptr<dawn::Expr>> initList, int dimension,
                                    const char* op) {
...
std::shared_ptr<dawn::StencilCallDeclStmt> scdec(const std::shared_ptr<dawn::sir::StencilCall>& stencilCall) {
...
std::shared_ptr<dawn::UnaryOperator> unop(const std::shared_ptr<dawn::Expr>& operand, const char* op) {
...
std::shared_ptr<dawn::VarAccessExpr> var(const std::string& name, std::shared_ptr<dawn::Expr> index) {
  return std::make_shared<dawn::VarAccessExpr>(name, index);
}

std::shared_ptr<dawn::FieldAccessExpr> field(const std::string& name, dawn::Array3i offset, dawn::Array3i argumentMap,
                                       dawn::Array3i argumentOffset, bool negateOffset) {
  return std::make_shared<dawn::FieldAccessExpr>(name, offset, argumentMap, argumentOffset, negateOffset);
}

std::shared_ptr<dawn::LiteralAccessExpr> lit(const std::string& value, dawn::BuiltinTypeID builtinType) {
