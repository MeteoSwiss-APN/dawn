...
public:
  ...
  virtual void visit(const std::shared_ptr<ReturnStmt>& stmt) override {
    GSL_ASSERT_MSG(scopeDepth_ == 1, "cannot inline nested return statement!");
    ...
    if(AccessIDOfCaller_ == 0) {
      ...
      auto newStmt = std::make_shared<VarDeclStmt>(
          gsl::Type(BuiltinTypeID::Float, CVQualifier::Const), returnVarName, 0, "=",
  ...
  void visit(const std::shared_ptr<StencilFunCallExpr>& expr) override {
    ...
    if(!argListScope_.empty()) {
      ...
      GSL_ASSERT(curFunc->isProvidedByStencilFunctionCall(AccessIDOfCaller));
    }
    ...
    if(inlineResult.first) {
      ...
      GSL_ASSERT(curStencilFunStmtIt != newStmtAccessesPairs_.end());
