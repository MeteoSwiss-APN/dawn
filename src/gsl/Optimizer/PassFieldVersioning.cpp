...
PassFieldVersioning::fixRaceCondition(const DependencyGraphAccesses* graph, Stencil& stencil,
  ...
  GSL_ASSERT_MSG(LHSAccessIDs.size() == 1, "left hand side should only have only one AccessID");
  ...
  GSL_ASSERT_MSG(stencilSCCs->size() == 1, "only one strongly connected component can be handled");
