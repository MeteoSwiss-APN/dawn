...
public:
  ...
  void visit(const std::shared_ptr<ReturnStmt>& stmt) override {
    GSL_ASSERT(scope_.top()->FunctionInstantiation);
  ...
  void visit(const std::shared_ptr<VerticalRegionDeclStmt>& stmt) override {
    GSL_ASSERT_MSG(0, "VerticalRegionDeclStmt not allowed in this context");
  }

  void visit(const std::shared_ptr<StencilCallDeclStmt>& stmt) override {
    GSL_ASSERT_MSG(0, "StencilCallDeclStmt not allowed in this context");
  }
  ...
  virtual void visit(const std::shared_ptr<StencilFunArgExpr>& expr) override {
    GSL_ASSERT(!scope_.top()->CandiateScopes.empty());
  ...
  void visit(const std::shared_ptr<VarAccessExpr>& expr) override {
    ...
    if(expr->isExternal()) {
      GSL_ASSERT(!function);
      GSL_ASSERT_MSG(!expr->isArrayAccess(), "global array access is not supported");
      ...
      if(value.isConstexpr()) {
        ...
        GSL_ASSERT_MSG(!value.empty(), "constant global variable with no value");

        auto newExpr = std::make_shared<gsl::LiteralAccessExpr>(
...
public:
  ...
  void tryReplaceStencilDescStmt(const std::shared_ptr<Stmt>& stencilDescNode) {
    GSL_ASSERT(stencilDescNode->isStencilDesc());
  ...
  void visit(const std::shared_ptr<ReturnStmt>& stmt) override {
    GSL_ASSERT_MSG(0, "ReturnStmt not allowed in this context");
  }
    ...
    if(evalExprAsBoolean(stmt->getCondExpr(), result, scope_.top()->VariableMap)) {
      ...
      if(scope_.top()->ScopeDepth == 1) {
        ...
        if(result) {
          ...
          GSL_ASSERT_MSG(thenBody, "then-body of if-statment should be a BlockStmt!");
          for(auto& s : thenBody->getStatements())
            s->accept(*this);
        } else if(stmt->hasElse()) {
          BlockStmt* elseBody = dyn_cast<BlockStmt>(stmt->getElseStmt().get());
          GSL_ASSERT_MSG(elseBody, "else-body of if-statment should be a BlockStmt!");
  ...
  void visit(const std::shared_ptr<VerticalRegionDeclStmt>& stmt) override {
    ...
    GSL_LOG(INFO) << "Processing vertical region at " << verticalRegion->Loc;
    ...
    GSL_LOG(INFO) << "Inserting statements ... ";
    ...
    GSL_LOG(INFO) << "Inserted " << doMethod.getStatementAccessesPairs().size() << " statements";
  ...
  void visit(const std::shared_ptr<StencilCallDeclStmt>& stmt) override {
    ...
    GSL_LOG(INFO) << "Processing stencil call to `" << stencilCall->Callee << "` at "
                  << stencilCall->Loc;
    ...
    GSL_ASSERT(stencilIt != instantiation_->getSIR()->Stencils.end());
    sir::Stencil& stencil = **stencilIt;

    // We need less or an equal amount of args as temporaries are added implicitly
    GSL_ASSERT(stencilCall->Args.size() <= stencil.Fields.size());
    ...
    GSL_LOG(INFO) << "Done processing stencil call to `" << stencilCall->Callee << "`";
  }

  void visit(const std::shared_ptr<BoundaryConditionDeclStmt>& stmt) override {
    GSL_ASSERT_MSG(0, "BoundaryConditionDeclStmt not yet implemented");
  }
  ...
  void visit(const std::shared_ptr<StencilFunCallExpr>& expr) override {
    GSL_ASSERT_MSG(0, "StencilFunCallExpr not allowed in this context");
  }
  void visit(const std::shared_ptr<StencilFunArgExpr>& expr) override {
    GSL_ASSERT_MSG(0, "StencilFunArgExpr not allowed in this context");
  }
...
StencilInstantiation::StencilInstantiation(OptimizerContext* context,
  ...
  GSL_LOG(INFO) << "Intializing StencilInstantiation of `" << SIRStencil->Name << "`";
  GSL_ASSERT_MSG(SIRStencil, "Stencil does not exist");
  ...
  GSL_LOG(INFO) << "Done initializing StencilInstantiation";
}
...
const sir::Value& StencilInstantiation::getGlobalVariableValue(const std::string& name) const {
  ...
  GSL_ASSERT(it != getSIR()->GlobalVariableMap->end());
  return *it->second;
}
...
void StencilInstantiation::promoteLocalVariableToTemporaryField(Stencil* stencil, int AccessID,
  ...
  GSL_ASSERT_MSG(varDeclStmt,
                 "first access to variable (i.e lifetime.Begin) is not an `VarDeclStmt`");
  GSL_ASSERT_MSG(!varDeclStmt->isArray(), "cannot promote local array to temporary field");
...
void StencilInstantiation::promoteTemporaryFieldToAllocatedField(int AccessID) {
  GSL_ASSERT(isTemporaryField(AccessID));
...
void StencilInstantiation::demoteTemporaryFieldToLocalVariable(Stencil* stencil, int AccessID,
  ...
  GSL_ASSERT_MSG(exprStmt, "first access of field (i.e lifetime.Begin) is not an `ExprStmt`");
  AssignmentExpr* assignmentExpr = dyn_cast<AssignmentExpr>(exprStmt->getExpr().get());
  GSL_ASSERT_MSG(assignmentExpr,
...
StencilFunctionInstantiation* StencilInstantiation::getStencilFunctionInstantiation(
  ...
  GSL_ASSERT_MSG(it != ExprToStencilFunctionInstantiationMap_.end(), "Invalid stencil function");
  return it->second;
}
...
int StencilInstantiation::getStencilIDFromStmt(
  ...
  GSL_ASSERT_MSG(it != StencilCallToStencilIDMap_.end(), "Invalid stencil call");
  return it->second;
}
