...
#ifndef GSL_OPTIMIZER_DEPENDENCYGRAPH_H
#define GSL_OPTIMIZER_DEPENDENCYGRAPH_H
...
public:
  ...
  std::size_t getVertexIDFromID(int ID) const {
    ...
    GSL_ASSERT_MSG(it != vertices_.end(), "Node with given ID does not exist");
  ...
  int getIDFromVertexID(std::size_t VertexID) const {
    ...
    gsl_unreachable("invalid VertexID");
  }
