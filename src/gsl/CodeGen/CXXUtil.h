...
#ifndef GSL_CODEGEN_CXXUTIL_H
#define GSL_CODEGEN_CXXUTIL_H
...
inline std::ostream& indent(int level, std::stringstream& ss) {
  ...
  default:
    return (ss << std::string(GSL_PRINT_INDENT * level, ' '));
  }
}
...
#define GSL_DECL_COMMIT(ClassName, Base)                                                           \
  ~ClassName() {                                                                                   \
    if(!isCommitted())                                                                             \
      commitImpl();                                                                                \
  }                                                                                                \
...
struct NewLine : public Streamable {
  ...
  void commitImpl() { ss() << "\n"; }
  GSL_DECL_COMMIT(NewLine, Streamable)
};
...
struct Statement : public NewLine {
  ...
  void commitImpl() { ss() << ";"; }
  GSL_DECL_COMMIT(Statement, NewLine)
};
...
struct Type : public Streamable {
  ...
  GSL_DECL_COMMIT(Type, Streamable);
};
...
struct Using : public Statement {
  ...
  void commitImpl() {}
  GSL_DECL_COMMIT(Using, Statement)
};
...
struct MemberFunction : public NewLine {
  ...
  MemberFunction& addInit(const Twine& name) {
    GSL_ASSERT(CanHaveInit && !IsBodyDeclared);
    finishArgs();
    ss() << (NumInits++ == 0 ? ": " : ", ") << name;
    return *this;
  }
  ...
  int getIndent() const { return GSL_PRINT_INDENT * (IndentLevel + 1); }
  ...
  GSL_DECL_COMMIT(MemberFunction, NewLine)
};
...
struct Structure : public Statement {
  ...
  GSL_DECL_COMMIT(Structure, Statement)
