...
#include "gsl/CodeGen/ASTCodeGenGTClangStencilDesc.h"
...
GTClangCodeGen::IntervalDefinitions::IntervalDefinitions(const Stencil& stencil)
  ...
  for(int i = 0; i < numStages; ++i) {
    ...
    GSL_ASSERT(iteratorSuccessPair.second);
  ...
  for(std::size_t stencilIdx = 0; stencilIdx < stencils.size(); ++stencilIdx) {
    ...
    for(const auto& stencilFun : stencilInstantiation->getStencilFunctionInstantiations()) {
      ...
      if(generatedStencilFun.emplace(stencilFunName).second) {
        ...
        GSL_ASSERT_MSG(intervalDefinitions.IntervalToNameMap.end() != interveralIt,
...
std::unique_ptr<TranslationUnit> GTClangCodeGen::generateCode() {
  ...
  if(mplContainerMaxSize_ > 20) {
    ...
    GSL_LOG(INFO) << "increasing boost::mpl template limit to " << mplContainerMaxSize_;
    ppDefines.push_back(defineIfNDef("BOOST_MPL_CFG_NO_PREPROCESSED_HEADERS", 1));
  }

  GSL_ASSERT_MSG(mplContainerMaxSize_ % 10 == 0,
                 "boost::mpl template limit needs to be multiple of 10");

  ppDefines.push_back(defineIfNDef("FUSION_MAX_VECTOR_SIZE", mplContainerMaxSize_));
  ppDefines.push_back(defineIfNDef("FUSION_MAX_MAP_SIZE", mplContainerMaxSize_));
  ppDefines.push_back(defineIfNDef("BOOST_MPL_LIMIT_VECTOR_SIZE", mplContainerMaxSize_));
