...
#ifndef GSL_SIR_SIR_H
#define GSL_SIR_SIR_H
...
struct Interval {
  ...
  Interval(int lowerLevel, int upperLevel, int lowerOffset = 0, int upperOffset = 0)
    ...
    GSL_ASSERT(lowerLevel >= LevelKind::Start && upperLevel <= LevelKind::End);
    GSL_ASSERT((lowerLevel + lowerOffset) <= (upperLevel + upperOffset));
  }
...
struct Value : NonCopyable {
  ...
  T getValue() const {
    GSL_ASSERT(!empty());
    GSL_ASSERT_MSG(TypeInfo<T>::Type == type_, "type missmatch");
