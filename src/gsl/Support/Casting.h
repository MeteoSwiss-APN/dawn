...
//===------------------------------------------------------------------------------------------===//
//
// This file is a port of the LLVM RTTI infrastructure.
// See: http://llvm.org/docs/HowToSetUpLLVMStyleRTTI.html
//
//===------------------------------------------------------------------------------------------===//

#ifndef GSL_SUPPORT_CASTING_H
#define GSL_SUPPORT_CASTING_H

#include "gsl/Support/Assert.h"
#include "gsl/Support/Compiler.h"
#include "gsl/Support/TypeTraits.h"
...
// The core of the implementation of isa<X> is here; To and From should be the names of classes.
// This template can be specialized to customize the implementation of isa<> without rewriting it
// from scratch.
...
struct isa_impl_cl<To, From*> {
  static inline bool doit(const From* Val) {
    GSL_ASSERT_MSG(Val, "isa<> used on a null pointer");
...
struct isa_impl_wrap {
  // When From != SimplifiedType, we can simplify the type some more by using the simplify_type
  // template.
...
// Calculate what type the 'cast' function should return, based on a requested  type of To and a
// source type of From.
template <class To, class From>
...
struct cast_retty_wrap {
  // When the simplified type and the from type are not the same, use the type simplifier to reduce
  // the type, then reuse cast_retty_impl to get the resultant type.
...
template <class To, class From>
struct cast_retty {
  typedef typename cast_retty_wrap<To, From, typename simplify_type<From>::SimpleType>::ret_type
      ret_type;
};

// Ensure the non-simple values are converted using the simplify_type template that may be
// specialized by smart pointers...
template <class To, class From, class SimpleFrom>
...
/// It does not allow a null argument (use cast_or_null for that).
...
cast(const Y& Val) {
  GSL_ASSERT_MSG(isa<X>(Val), "cast<Ty>() argument of incompatible type!");
...
cast_or_null(const Y& Val) {
  if(!Val)
    return nullptr;
  GSL_ASSERT_MSG(isa<X>(Val), "cast_or_null<Ty>() argument of incompatible type!");
  return cast<X>(Val);
}
...
/// @brief  Return the argument parameter cast to the specified type.
///
/// This casting operator returns null if the argument is of the wrong type, so it can be used to
/// test for a type as well as cast if successful. This should be used in the context of an if
/// statement like this:
///
/// @code
