...
//===------------------------------------------------------------------------------------------===//
//
// This file is a port of LLVM's SmallVector.
//
//===------------------------------------------------------------------------------------------===//

#ifndef GSL_SUPPORT_SMALLVECTOR_H
#define GSL_SUPPORT_SMALLVECTOR_H

#include "gsl/Support/AlignOf.h"
#include "gsl/Support/Assert.h"
#include "gsl/Support/Compiler.h"
#include "gsl/Support/MathExtras.h"
...
/// @brief This is the part of SmallVectorTemplateBase which does not depend on whether the type T
/// is a POD. The extra dummy template argument is used by ArrayRef to avoid unnecessarily requiring
/// T to be complete.
...
public:
  ...
  /// @brief Return the total number of elements in the currently allocated buffer
  size_t capacity() const { return capacity_ptr() - begin(); }

  /// @brief Return a pointer to the vector's buffer, even if empty()
...
public:
  void push_back(const T& Elt) {
    if(GSL_BUILTIN_UNLIKELY(this->EndX >= this->CapacityX))
      this->grow();
...
public:
  ...
  iterator erase(const_iterator CI) {
    ...
    GSL_ASSERT(I >= this->begin() && "Iterator to erase is out of bounds.");
    GSL_ASSERT(I < this->end() && "Erasing at past-the-end iterator.");
  ...
  iterator erase(const_iterator CS, const_iterator CE) {
    ...
    GSL_ASSERT_MSG(S >= this->begin(), "Range to erase is out of bounds.");
    GSL_ASSERT_MSG(S <= E, "Trying to erase invalid range.");
    GSL_ASSERT_MSG(E <= this->end(), "Trying to erase past the end.");
  ...
  iterator insert(iterator I, T&& Elt) {
    ...
    GSL_ASSERT_MSG(I >= this->begin(), "Insertion iterator is out of bounds.");
    GSL_ASSERT_MSG(I <= this->end(), "Inserting past the end of the vector.");
...
template <typename T>
inline void swap(gsl::SmallVectorImpl<T>& LHS, gsl::SmallVectorImpl<T>& RHS) {
  LHS.swap(RHS);
}

/// @brief Implement std::swap in terms of SmallVector swap
template <typename T, unsigned N>
inline void swap(gsl::SmallVector<T, N>& LHS, gsl::SmallVector<T, N>& RHS) {
  LHS.swap(RHS);
}
