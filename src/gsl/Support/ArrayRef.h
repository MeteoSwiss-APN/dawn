...
#ifndef GSL_SUPPORT_ARRAYREF_H
#define GSL_SUPPORT_ARRAYREF_H
...
public:
  ...
  ArrayRef<T> slice(size_t N, size_t M) const {
    GSL_ASSERT_MSG(N + M <= size(), "Invalid specifier");
  ...
  ArrayRef<T> drop_front(size_t N = 1) const {
    GSL_ASSERT_MSG(size() >= N, "Dropping more elements than exist");
  ...
  const T& operator[](size_t Index) const {
    GSL_ASSERT_MSG(Index < length_, "Invalid index!");
    return data_[Index];
  }
...
public:
  ...
  MutableArrayRef<T> slice(size_t N, size_t M) const {
    GSL_ASSERT_MSG(N + M <= this->size(), "Invalid specifier");
  ...
  MutableArrayRef<T> drop_front(size_t N = 1) const {
    GSL_ASSERT_MSG(this->size() >= N, "Dropping more elements than exist");
  ...
  T& operator[](size_t Index) const {
    GSL_ASSERT_MSG(Index < this->size(), "Invalid index!");
    return data()[Index];
  }
};
