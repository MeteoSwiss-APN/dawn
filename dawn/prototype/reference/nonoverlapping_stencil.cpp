...
private:
  ...
  public:
    ...
    stencil_182(const gridtools::dawn::domain& dom_, int rank, int xcols, int ycols) : m_dom(dom_){}
        ...
        for(int j = jMin+0; j  <=  jMax+0; ++j) {
{
  ...
  stencil_182 m_stencil_182;
public:

  generated(const generated&) = delete;

  generated(const gridtools::dawn::domain& dom, int rank = 1, int xcols = 1, int ycols = 1) : m_stencil_182(dom, rank, xcols, ycols){
