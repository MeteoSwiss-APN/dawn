...
__global__ void __launch_bounds__(128) generated_stencil28_ms27_kernel(
    const int isize, const int jsize, const int ksize, const int stride_111_1,
    const int stride_111_2, ::dawn::float_type* const in_field, ::dawn::float_type* const out_field,
    int* const stage14GlobalJIndices, unsigned* const globalOffsets) {
  ...
  for(int k = kleg_lower_bound + 0; k <= kleg_upper_bound + 0; ++k) {
    ...
    if(iblock >= 0 && iblock <= block_size_i - 1 + 0 && jblock >= 0 &&
       jblock <= block_size_j - 1 + 0 &&
       checkOffset(stage14GlobalJIndices[0], stage14GlobalJIndices[1], globalOffsets[1] + jblock)) {
      {
    ...
    void run(storage_ijk_t in_field_ds, storage_ijk_t out_field_ds) {
      ...
      start();
      {
        ...
        generated_stencil28_ms27_kernel<<<blocks, threads>>>(
            ...
            stage14GlobalJIndices.data(), globalOffsets.data());
      };
