...
def create_indexed_vr(i_interval=None, j_interval=None) -> VerticalRegionDeclStmt:
    interval = make_interval(Interval.Start, Interval.End, 0, 0)
    boundary_body = make_ast(
        [
            # Stage splitting works
            # make_assignment_stmt(
            #     make_field_access_expr("in", [0, 0, 0]), make_literal_access_expr("1", BuiltinType.Float), "="
            # ),
            # make_assignment_stmt(
            #     make_field_access_expr("out", [0, 0, 0]), make_field_access_expr("in", [1, 0, 0]), "="
            # ),
            ### FV fix should catch
            # make_assignment_stmt(
            #     make_field_access_expr("in", [0, 0, 0]), make_field_access_expr("out", [0, 0, 0]), "="
            # ),
            # make_assignment_stmt(
            #     make_field_access_expr("out", [0, 0, 0]), make_field_access_expr("in", [1, 0, 0]), "="
            # ),
            ### check if in two stages requires FV?
            make_assignment_stmt(
                make_field_access_expr("in", [0, 0, 0]), make_field_access_expr("out", [0, 0, 0]), "="
            ),
        ]
...
def create_indexed_vr2(i_interval=None, j_interval=None) -> VerticalRegionDeclStmt:
    interval = make_interval(Interval.Start, Interval.End, 0, 0)
    boundary_body = make_ast(
        ...
        make_stencil(
            ...
            make_ast(
                [
                    ...
                    create_indexed_vr(i_interval=make_interval(Interval.End, Interval.End, -1, 0)),
                    create_indexed_vr2(i_interval=make_interval(Interval.Start, Interval.Start, 0, 1)),
                ]
            ),
