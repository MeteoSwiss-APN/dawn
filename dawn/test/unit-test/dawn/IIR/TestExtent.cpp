...
TEST(ExtentsTest, Construction) {
  Extents extents(ast::Offset{{-1, 1, 0}});
...
TEST(ExtentsTest, Merge1) {
  Extents extents(ast::Offset{{-1, 1, 0}});
  Extents extentsToMerge(ast::Offset{{3, 2, 1}});
...
TEST(ExtentsTest, Merge2) {
  Extents extents(ast::Offset{{-1, 1, 0}});
  Extents extentsToMerge(ast::Offset{{-2, 2, 0}});
  extents.merge(extentsToMerge);
...
TEST(ExtentsTest, Merge3) {
  Extents extents(ast::Offset{{-1, 1, 0}});
  extents.merge(ast::Offset{{-2, 0, 0}});
...
TEST(ExtentsTest, Stringify) {
  Extents extents(ast::Offset{{1, -1, 2}});
  std::stringstream ss;
  ss << extents;
