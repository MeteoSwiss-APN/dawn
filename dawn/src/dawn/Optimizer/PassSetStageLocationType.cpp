...
ast::LocationType deduceLocationType(const std::shared_ptr<ast::Stmt>& stmt,
  ...
  if(const auto& exprStmt = std::dynamic_pointer_cast<ast::ExprStmt>(stmt)) {
    if(const auto& assignmentExpr =
      ...
      if(const std::shared_ptr<ast::FieldAccessExpr> fieldAccessExpr =
             std::dynamic_pointer_cast<ast::FieldAccessExpr>(assignmentExpr->getLeft())) {

        return metaInformation.getDenseLocationTypeFromAccessID(iir::getAccessID(fieldAccessExpr));

      } else if(const std::shared_ptr<ast::VarAccessExpr> varAccessExpr =
                    std::dynamic_pointer_cast<ast::VarAccessExpr>(assignmentExpr->getLeft())) {

        return metaInformation.getLocalVariableDataFromAccessID(iir::getAccessID(varAccessExpr))
            .getLocationType();
      }
    } else if(const std::shared_ptr<ast::StencilFunCallExpr> stencilFunCallExpr =
                  std::dynamic_pointer_cast<ast::StencilFunCallExpr>(exprStmt->getExpr())) {
  ...
  } else if(const auto& varDeclStmt = std::dynamic_pointer_cast<ast::VarDeclStmt>(stmt)) {
  ...
  } else if(const auto& loopStmt = std::dynamic_pointer_cast<ast::LoopStmt>(stmt)) {
    if(auto* chainDesc =
