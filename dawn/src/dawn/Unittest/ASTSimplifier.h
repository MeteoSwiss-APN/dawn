...
public:
  ...
  void recursiveBlock(const std::shared_ptr<sir::Expr>& expression, Args&&... args) {
    recursiveBlock(sir::makeExprStmt(expression), std::forward<Args>(args)...);
  }

  void recursiveBlock() {}

  template <typename... Args>
  const std::vector<std::shared_ptr<sir::Stmt>>&
  createVec(const std::shared_ptr<sir::Stmt>& statement, Args&&... args) {
...
std::shared_ptr<sir::VarDeclStmt> vardecl(const std::string& type, const std::string& name,
                                          const std::shared_ptr<sir::Expr>& init,
                                          const char* op = "=");
...
std::shared_ptr<sir::VarAccessExpr> var(const std::string& name,
                                        std::shared_ptr<sir::Expr> index = nullptr);
std::shared_ptr<sir::FieldAccessExpr> field(const std::string& name,
                                            Array3i offset = Array3i{{0, 0, 0}},
