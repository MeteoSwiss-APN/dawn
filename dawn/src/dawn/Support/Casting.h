...
struct isa_impl_cl<To, From*> {
  static inline bool doit(const From* Val) {
    DAWN_ASSERT_MSG(Val, "isa<> used on a null pointer");
...
cast(const Y& Val) {
  DAWN_ASSERT_MSG(isa<X>(Val), "cast<Ty>() argument of incompatible type!");
...
cast_or_null(const Y& Val) {
  if(!Val)
    return nullptr;
  DAWN_ASSERT_MSG(isa<X>(Val), "cast_or_null<Ty>() argument of incompatible type!");
  return cast<X>(Val);
}
