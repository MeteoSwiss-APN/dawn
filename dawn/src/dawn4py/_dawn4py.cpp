...
PYBIND11_MODULE(_dawn4py, m) {
  ...
  py::class_<dawn::Options>(m, "Options")
      .def(
          py::init([](int MaxHaloSize, bool UseParallelEP, int MaxBlocksPerSM, int nsms,
                      int DomainSizeI, int DomainSizeJ, int DomainSizeK, const std::string& Backend,
                      const std::string& OutputFile, bool SerializeIIR, bool DeserializeIIR,
                      const std::string& IIRFormat, int MaxHaloPoints,
                      ...
                      bool SetCaches, bool SetBlockSize, bool SetLoopOrder, bool DataLocalityMetric,
          ...
          py::arg("stage_merger") = true, py::arg("temporary_merger") = false,
      ...
      .def("__repr__", [](const dawn::Options& self) {
        ...
        ss << "max_halo_size=" << self.MaxHaloSize << ",\n    "
           ...
           << "deserialize_iir=" << self.DeserializeIIR << ",\n    "
           << "iir_format="
           << "\"" << self.IIRFormat << "\""
           << ",\n    "
