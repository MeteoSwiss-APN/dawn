...
TEST(SIRComparisonTest, DoubleReference) {
  ...
  auto arg01 = std::make_shared<StencilFunctionArg>();
  arg01->Name = "foo";
  arg01->Kind = StencilFunctionArg::ArgumentKind::AK_Field;
  ...
  auto interval01 = std::make_shared<Interval>(0, 1);
  ...
  auto ast01 = std::make_shared<AST>(std::make_shared<BlockStmt>(std::vector<std::shared_ptr<Stmt>>{
      std::make_shared<ExprStmt>(std::make_shared<VarAccessExpr>("foo"))}));
  ...
  val.get()->setType(Value::TypeKind::Integer);
  val.get()->setValue(10);
