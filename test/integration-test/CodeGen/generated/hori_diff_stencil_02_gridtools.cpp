 ...
 public:
  struct stencil_0 {
    ...
    struct laplacian_interval_start_0_end_0 {
      ...
      using phi = gridtools::accessor<1, gridtools::enumtype::in, gridtools::extent<-1, 1, -1, 1, 0, 0>>;
      using arg_list = boost::mpl::vector<__out, phi>;

      template <typename Evaluation>
      GT_FUNCTION static void Do(Evaluation& eval, interval_start_0_end_0) {
        eval(__out(0, 0, 0)) =
            ((((eval(phi(1, 0, 0)) + eval(phi(-1, 0, 0))) + eval(phi(0, 1, 0))) + eval(phi(0, -1, 0))) -
             ((gridtools::clang::float_type)4 * eval(phi(0, 0, 0))));
      }
    };
    ...
    struct stage_0_0 {
      ...
      GT_FUNCTION static void Do(Evaluation& eval, interval_start_0_end_0) {
        eval(lap(0, 0, 0)) =
            gridtools::call<laplacian_interval_start_0_end_0, interval_start_0_end_0>::with(eval, u(0, 0, 0));
      }
    };
    ...
    struct stage_0_1 {
      ...
      GT_FUNCTION static void Do(Evaluation& eval, interval_start_0_end_0) {
        eval(out(0, 0, 0)) =
            gridtools::call<laplacian_interval_start_0_end_0, interval_start_0_end_0>::with(eval, lap(0, 0, 0));
      }
    };
