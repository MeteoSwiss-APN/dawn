...
function(dawn_protobuf_generate)
  ...
  set(multi_value_args PROTOS PROTO_PATH)
  ...
  message("setting proto path to `${ARG_PROTO_PATH}`")
  if(ARG_PROTO_PATH)
    set(PRTOPATH_INCLUDE "--proto_path=${ARG_PROTO_PATH}")
  endif()
  message("now: PRTOPATH_INCLUDE: `${PRTOPATH_INCLUDE}`")
  ...
  message("now we have: ${PRTOPATH_INCLUDE} ")
  file(APPEND "${protobuf_script}" " ${PRTOPATH_INCLUDE} $*\n")
  file(READ ${protobuf_script} tempvar)
  message("and the script: ${protobuf_script}:\n${tempvar}")
  ...
  foreach(proto ${ARG_PROTOS})
    ...
    foreach(src ${generated_srcs})
      ...
      if(${existing} EQUAL -1)
        list(APPEND output_include_dirs -I${abs_path})
      endif()
    endforeach()
  endforeach()
