...
int __local_m_99 = ((gridtools::clang::float_type) 1.0 / (__ldg(&(b[idx111])) - (__ldg(&(a[idx111])) * c_kcache[0])));
c_kcache[1] = (c_kcache[1] * __local_m_99);
d_kcache[1] = ((d_kcache[1] - (__ldg(&(a[idx111])) * d_kcache[0])) * __local_m_99);
  }
    // Flush of kcaches
...
__global__ void __launch_bounds__(32)  tridiagonal_solve_stencil93_ms104_kernel(const int isize, const int jsize, const int ksize, const int stride_111_1, const int stride_111_2, gridtools::clang::float_type * const c, gridtools::clang::float_type * const d) {
...
public:
  ...
  struct stencil_93 : public sbase {
  ...
  public:
    ...
    stencil_93(const gridtools::clang::domain& dom_, storage_ijk_t& a_, storage_ijk_t& b_, storage_ijk_t& c_, storage_ijk_t& d_) : sbase("stencil_93"), m_dom(dom_), m_a(a_), m_b(b_), m_c(c_), m_d(d_){}
    ...
    virtual void run() {
      ...
      tridiagonal_solve_stencil93_ms103_kernel<<<blocks, threads>>>(nx,ny,nz,m_a.strides()[1],m_a.strides()[2],(a.data()+m_a.get_storage_info_ptr()->index(a.begin<0>(), a.begin<1>(),0 )),(b.data()+m_b.get_storage_info_ptr()->index(b.begin<0>(), b.begin<1>(),0 )),(c.data()+m_c.get_storage_info_ptr()->index(c.begin<0>(), c.begin<1>(),0 )),(d.data()+m_d.get_storage_info_ptr()->index(d.begin<0>(), d.begin<1>(),0 )));
      };
      {;
      gridtools::data_view<storage_ijk_t> c= gridtools::make_device_view(m_c);
      gridtools::data_view<storage_ijk_t> d= gridtools::make_device_view(m_d);
      ...
      const unsigned int nbz = 1;
      dim3 blocks(nbx, nby, nbz);
      tridiagonal_solve_stencil93_ms104_kernel<<<blocks, threads>>>(nx,ny,nz,m_c.strides()[1],m_c.strides()[2],(c.data()+m_c.get_storage_info_ptr()->index(c.begin<0>(), c.begin<1>(),0 )),(d.data()+m_d.get_storage_info_ptr()->index(d.begin<0>(), d.begin<1>(),0 )));
      };
...
public:
  ...
  tridiagonal_solve(const gridtools::clang::domain& dom, storage_ijk_t& a, storage_ijk_t& b, storage_ijk_t& c, storage_ijk_t& d) : m_stencil_93(new stencil_93(dom,a,b,c,d) ){}
...
;
  ...
  std::vector<sbase*> getStencils() {
    return std::vector<sbase*>({m_stencil_93});
  }
