...
public:
  ...
  public:
    ...
    stencil_41(const gridtools::clang::domain& dom_, storage_ijk_t& in_, storage_ijk_t& out_, storage_ijk_t& coeff_) : sbase("stencil_41"), m_dom(dom_), m_in(in_), m_out(out_), m_coeff(coeff_), m_tmp_meta_data(32+2, 4+2, (dom_.isize()+ 32 - 1) / 32, (dom_.jsize()+ 4 - 1) / 4, dom_.ksize() + 2 * 0), m_lap(m_tmp_meta_data){}
    ...
    virtual void run() {
      ...
      hori_diff_stencil41_ms54_kernel<<<blocks, threads>>>(nx,ny,nz,m_in.strides()[1],m_in.strides()[2],(in.data()+m_in.get_storage_info_ptr()->index(in.begin<0>(), in.begin<1>(),0 )),(out.data()+m_out.get_storage_info_ptr()->index(out.begin<0>(), out.begin<1>(),0 )),(coeff.data()+m_coeff.get_storage_info_ptr()->index(coeff.begin<0>(), coeff.begin<1>(),0 )));
      };
...
;
  ...
  std::vector<sbase*> getStencils() {
    return std::vector<sbase*>({m_stencil_41});
  }
