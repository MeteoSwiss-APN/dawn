...
static const char* BadStrings[] = {"" // empty string
                                   ,
...
TEST(StringRefTest, consumeIntegerUnsigned) {
  ...
  for(size_t i = 0; i < dawn::array_lengthof(ConsumeUnsigned); ++i) {
    ...
    if(static_cast<uint16_t>(ConsumeUnsigned[i].Expected) == ConsumeUnsigned[i].Expected) {
      ASSERT_FALSE(U16Success);
    ...
    if(static_cast<uint32_t>(ConsumeUnsigned[i].Expected) == ConsumeUnsigned[i].Expected) {
      ASSERT_FALSE(U32Success);
    ...
    if(static_cast<uint64_t>(ConsumeUnsigned[i].Expected) == ConsumeUnsigned[i].Expected) {
      ASSERT_FALSE(U64Success);
...
TEST(StringRefTest, consumeIntegerSigned) {
  ...
  for(size_t i = 0; i < dawn::array_lengthof(ConsumeSigned); ++i) {
    ...
    if(static_cast<int16_t>(ConsumeSigned[i].Expected) == ConsumeSigned[i].Expected) {
      ASSERT_FALSE(S16Success);
    ...
    if(static_cast<int32_t>(ConsumeSigned[i].Expected) == ConsumeSigned[i].Expected) {
      ASSERT_FALSE(S32Success);
    ...
    if(static_cast<int64_t>(ConsumeSigned[i].Expected) == ConsumeSigned[i].Expected) {
      ASSERT_FALSE(S64Success);
