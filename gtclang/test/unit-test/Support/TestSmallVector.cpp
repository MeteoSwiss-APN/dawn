...
public:
  ...
  friend bool DAWN_ATTRIBUTE_UNUSED operator!=(const Constructable& c0, const Constructable& c1) {
    return c0.getValue() != c1.getValue();
  }
};
...
TYPED_TEST(SmallVectorTest, PushPopTest) {
  ...
  if(!RequiresGrowth) {
    ...
    EXPECT_EQ(Constructable::getNumConstructorCalls(), Constructable::getNumDestructorCalls());
  }
}
...
TYPED_TEST(SmallVectorTest, MoveAssignTest) {
  ...
  EXPECT_EQ(Constructable::getNumConstructorCalls() - 2, Constructable::getNumDestructorCalls());
...
TYPED_TEST(SmallVectorTest, InsertCopy) {
  ...
  this->makeSequence(this->theVector, 1, 3);
  Constructable C(77);
  typename TypeParam::iterator I = this->theVector.insert(this->theVector.begin() + 1, C);
  EXPECT_EQ(this->theVector.begin() + 1, I);
  this->assertValuesInOrder(this->theVector, 4u, 1, 77, 2, 3);
}
...
TYPED_TEST(SmallVectorTest, InsertRepeatedTest) {
  ...
  Constructable::reset();
  auto I = this->theVector.insert(this->theVector.begin() + 1, 2, Constructable(16));
...
TYPED_TEST(SmallVectorTest, InsertEmptyRangeTest) {
  ...
  EXPECT_EQ(this->theVector.begin() + 1,
            this->theVector.insert(this->theVector.begin() + 1, this->theVector.begin(),
                                   this->theVector.begin()));
}
...
TYPED_TEST(DualSmallVectorsTest, MoveAssignment) {
  ...
  // Move-assign from the other vector.
  this->theVector = std::move(static_cast<SmallVectorImpl<Constructable>&>(this->otherVector));
  ...
  // are two live objects after clearing the other vector.
  this->otherVector.clear();
  EXPECT_EQ(Constructable::getNumConstructorCalls() - 4, Constructable::getNumDestructorCalls());
